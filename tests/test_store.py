...
PRICE_TABLE = pa.Table.from_arrays([symbol, timestamp, open, high, close, low, volume], schema=OHCL)
...
SYMBOL_TABLE = pa.Table.from_arrays([ticker, figi, type, mic, description, currency_name], schema=STOCK_SYMBOL)


def test_save_and_load(tmp_path):
    folder = tmp_path / ""
    store = PriceStore(str(folder))
    store.store(symbol_table=SYMBOL_TABLE, price_table=PRICE_TABLE, start=START, end=END, frequency="D", vendor="test")
    price_table = store.load_prices(start=START, end=END, frequency="D", vendor="test")
